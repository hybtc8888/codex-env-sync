...
**Windows local setup note**: If `.venv\Scripts\python.exe` exists, use it instead of `python3`. This keeps the skill runnable when the global Python environment is missing dependencies or `pip install` is network-restricted.
...
- Python 3.8+. Recommended on Windows:
  - `python -m venv .venv`
  - `.venv\Scripts\python.exe -m pip install -r requirements.txt`
  - run commands with `.venv\Scripts\python.exe scripts\fetch_news.py ...`
